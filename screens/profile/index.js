...
export default function Profile({ navigation, route }) {
    ...
    const getUser = async () => {
      try {
        const response = await axios.get(
          `https://coffeshop-mobile.up.railway.app/api/v1/users/${userID}`
        );
  ...
  useEffect(() => {
    const getHistoryAPI = async () => {
      try {
        const response = await axios.get(
          `https://coffeshop-mobile.up.railway.app/api/v1/history/${userID}`
        );
        const data = await response?.data?.data;
        setHistory(data);
      } catch (error) {
        ...
        <View style={styles.profilePicWrapper}>
          <Image
            source={
              userData?.image
                ? {
                    uri: `https://coffeshop-mobile.up.railway.app/uploads/images/${userData?.image}`,
                  }
